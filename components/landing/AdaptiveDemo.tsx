...
export function AdaptiveDemo() {
  ...
  return (
    <section
      className={`${styles.divineSection} ${styles.sectionAnchor} ${styles.demoSection}`}
      style={{
        zIndex: 4,
        background:
          "radial-gradient(ellipse 110% 420px at 50% -90px, rgba(236,182,210,.055), rgba(169,155,217,.03) 44%, rgba(127,208,196,.014) 62%, transparent 78%), radial-gradient(ellipse 92% 360px at 50% 340px, rgba(169,155,217,.1), rgba(236,182,210,.045) 42%, transparent 76%), linear-gradient(180deg,#070512 0%,#08060f 20%,#090715 54%,#070512 88%,#070512 100%)",
        borderTop: 0,
      }}
    >
      <div className={styles.constellation} aria-hidden="true" />
      <AuroraGlow style={{ top: -120, right: "12%", width: 520, height: 520 }} />
      <div className={styles.sectionInner} style={{ maxWidth: 1080, margin: "0 auto", padding: "clamp(72px,9vw,108px) clamp(20px,5vw,32px) clamp(52px,9vw,104px)" }}>
        ...
        <div className={styles.oracleLayout} style={{ marginTop: 56, display: "flex", gap: 44, alignItems: "center", justifyContent: "center", flexWrap: "wrap" }}>
          <div className={styles.voiceOrb} style={{ position: "relative", width: 200, height: 200, flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center" }}>
          ...
          <div className={`${styles.card} ${styles.oracleCard}`} style={{ flex: 1, minWidth: 300, maxWidth: 560, background: "#11101E", border: "1px solid rgba(255,255,255,.09)", borderRadius: 22, padding: "34px 34px 30px", boxShadow: "0 30px 70px -36px rgba(0,0,0,.7)" }}>
            <div style={{ position: "relative", font: "600 12px var(--font-sans)", letterSpacing: "1.8px", textTransform: "uppercase", color: "#827C99" }}>
              Pra quem <span style={{ color: "var(--accent)", ...fade }}>{s.who}</span>
            </div>
            <div className="font-serif" style={{ position: "relative", marginTop: 14, fontSize: "clamp(22px,3.2vw,27px)", fontWeight: 450, lineHeight: 1.32, letterSpacing: "-0.01em", color: "#F0ECF7", textWrap: "pretty", minHeight: 72, ...fade }}>
              {s.q}
            </div>
            <div style={{ position: "relative", marginTop: 18, display: "flex", alignItems: "center", gap: 9, color: "#6F6987", font: "500 12px var(--font-sans)", letterSpacing: ".3px" }}>
