...
const INCOMPLETE_CAMPAIGN_KEY = "alpha_still_time_incomplete_2026_06_26";
const COMPLETE_CAMPAIGN_KEY = "alpha_still_time_complete_2026_06_26";
const CONFIRM_SEND = "send-alpha-still-time-2026-06-26";
...
function campaignForSegment(segment: CampaignSegment) {
  return segment === "ritual_complete" ? COMPLETE_CAMPAIGN_KEY : INCOMPLETE_CAMPAIGN_KEY;
}

function eventBaseForSegment(segment: CampaignSegment) {
  return segment === "ritual_complete" ? "alpha_still_time_complete" : "alpha_still_time_incomplete";
}
...
async function selectIncompleteCandidates(limit: number) {
        ...
        and not exists (
          ...
          where sent_event.waitlist_id = w.id
            and sent_event.event_name in (
              ...
              'alpha_still_time_incomplete_email_scheduled',
              'alpha_still_time_incomplete_email_send_failed'
            )
            and sent_event.metadata->>'campaign' = ${INCOMPLETE_CAMPAIGN_KEY}
        )
        ...
        and not exists (
          ...
          where sent_event.waitlist_id = w.id
            and sent_event.event_name in (
              ...
              'alpha_still_time_complete_email_scheduled',
              'alpha_still_time_complete_email_send_failed'
            )
...
async function runAlphaBroadcast(request: Request) {
  ...
  if (testTo) {
    ...
    const sent = dryRun
      ...
      : segment === "ritual_complete"
        ? await sendAlphaTesterCompleteEmail(candidate, baseUrl, scheduledAt)
  ...
  const candidates = segment === "ritual_complete"
    ...
    : await selectIncompleteCandidates(limit);
  const baseUrl = siteUrl(request.url);
